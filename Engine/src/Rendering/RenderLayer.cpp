...
namespace Refraction::Engine {
	RenderLayer::RenderLayer(Common::SRef<Events::AEventDispatcher> eventDispatcher, Common::SRef<Project> projectInstance)
	...
	void RenderLayer::OnPass() {
		if (mProjectInstance->IsLoaded()) {
			...
			mEventDispatcher->Dispatch(Common::NewSRef<Events::FrameRenderedEvent>(mRenderer.GetFinalOutput()));
		}
	}

	void RenderLayer::OnEvent(Common::SRef<Events::Event> event) {
