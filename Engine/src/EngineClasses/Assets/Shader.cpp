...
namespace Refraction::Assets {
	...
	void Shader::LoadAllShaders() {
		...
		std::vector<directory_entry> shaderSources = Utilities::GetFoldersInFolder(Constants::GetResourcePath() + shadersSubPath);

		for (const auto& shaderSource : shaderSources) {
			std::string shaderSourcePath = shaderSource.path().string();
			RenderLog::Info("Loading shader source: " + shaderSourcePath);
			Shader* newShader = new Shader(shaderSourcePath);
	...
	void Shader::SetUniformVec2(const std::string& name, const Vector2& value) const {
	...
	void Shader::SetUniformVec3(const std::string& name, const Vector3& value) const {
